...
nodi <- c("roll1_disc","accel1_disc","accel2_disc","roll2_disc","roll3_disc","roll4_disc","pitch2_disc","pitch3_disc","pitch4_disc","accel_mean_disc","accel_std_disc", "sitting","sittingdown","walking","standing","standingup")
e = empty.graph(nodi)
class(e)
e
...
corrplot(cor(newdata, method = "pearson"), type = 'lower')

write.csv(newdata, file = "dataset_x_corr.csv", row.names=FALSE)

toRemove <- findCorrelation(cor(newdata), cutoff = 0.7)
corrplot(cor(newdata[,-toRemove]), type = 'lower')
#_________________
for (col in nodi){
...
for (i in 1:nrow(test_fit)) {
        ...
        prob_sitting <- cpquery(fit, sitting == 1, roll1_disc == roll1 & roll2_disc == roll2 & roll3_disc == roll3 & 
                        roll4_disc == roll4 & pitch2_disc == pitch2 & pitch3_disc == pitch3 & pitch4_disc == pitch4 &
                        accel1_disc == accel1 & accel2_disc == accel2 & accel_mean_disc == accel_mean & accel_std_disc == accel_std)
